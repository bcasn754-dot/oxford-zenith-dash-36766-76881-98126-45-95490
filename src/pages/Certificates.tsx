      ...
      <div className="p-8 space-y-8 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {certificates.map((certificate) => (
            <Card
              key={certificate.id}
              className="p-8 shadow-elegant hover:shadow-hover transition-all animate-scale-in bg-gradient-subtle border-border"
            >
              <div className="flex flex-col items-center text-center mb-6">
                <div className="w-20 h-20 rounded-full bg-gradient-gold flex items-center justify-center mb-4 shadow-lg">
                  <Award className="w-10 h-10 text-oxford" />
                </div>
                <h2 className="text-2xl font-bold mb-2 text-foreground">Certificate of Completion</h2>
                <div className="w-16 h-1 bg-gradient-gold rounded-full mb-4"></div>
                <p className="text-lg font-semibold mb-2 text-foreground">{certificate.title}</p>
                <p className="text-sm text-muted-foreground">{certificate.course}</p>
              </div>

              <div className="space-y-3 mb-6 bg-muted/50 rounded-lg p-4 border border-border">
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-bold text-lg text-gold">{certificate.grade}%</span>
                </div>
                <div className="flex items-center justify-between text-sm">
